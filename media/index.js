    ...
    initialize(view) {
        ...
        if (openFileButton && openFileDialog) {
            openFileButton.addEventListener('click', () => {
                ...
                //     //这里可以是图片、音频、视频、文本文件等等前端可以选择的文件
                //     xhr.open("get", "/linear_mnist_model-5000.meta", true);
                ...
                //             //可以把对象赋给相应的页面控件
                //             let files = []
                //             files.push(new window.File([blob], 'linear_mnist_model-5000.meta'))
                //             // var files = Array.from(new window.File([blob], 'test'));
                //             //  var file = files.find((file) => this._view.accept(file.name));
                //             console.log(files)
                //             if (files) {
                //                 that._open(files[0], files);
                //             }
                //         }
                //     }
                //     xhr.send();
